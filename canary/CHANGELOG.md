...
### Added
- **UK identity detector pack (2 new regex types, 36 → 38).** The product of broad research into open-source PII-redaction tooling — chiefly the [redacta](https://github.com/nickjlamb/redacta) clinical de-identification project, whose UK detector set Canary's regex layer only half-covered: it already caught NHS numbers but neither of the two identifiers a UK address or tax record almost always carries.
  - **`uk_nino`** — UK National Insurance number, the UK's closest analogue to a US SSN and a long-standing gap next to the NHS number Canary already caught (it was even the first item on CONTRIBUTING.md's detector wishlist). Validated with HMRC's published structural rules in `nino_valid()`: a two-letter prefix that excludes the disallowed first letters (`D F I Q U V`), disallowed second letters (`D F I O Q U V`), and never-allocated prefixes (`BG GB NK KN TN NT ZZ`), followed by six digits and an `A`–`D` suffix. A NINO has no check digit, so — like `us_ssn`/`us_itin` — this is a range/exclusion guarantee, not a checksum: it fires `"high"` with a national-insurance keyword nearby and `"medium"` on the validated shape alone. The GOV.UK example `QQ123456C` is rejected for free (Q is an invalid first letter), so it needs no placeholder-denylist entry.
  - **`uk_postcode`** — full UK postcode, PII under UK-GDPR since a full postcode narrows to roughly 15 households. Format-only (Royal Mail postcodes carry no check digit), so it is `"medium"` for the distinctive spaced form and `"high"` only when an address/postcode keyword confirms the context. The ambiguous *unspaced* form is deliberately suppressed unless a keyword is present, so it can't collide with hex fragments, build artifacts, product codes, and other ordinary alphanumeric tokens.
  - Both are wired into the shared `taxonomy.json` (`uk_nino` → Identity, `uk_postcode` → Network, both tagged `UK-GDPR`), so every scoring surface — the dashboard, HUD, `canary-stats`, `canary-card`, `canary-badge`, and Canary Wrapped — grades and groups them with no further change. The Stop-hook LLM "skip" list now owns them too, so a NINO is never double-counted as `national_id`, nor a postcode as `zip_code`.
- Test coverage for the pack: `test-detectors.sh` (+11 assertions), `test-checksums.sh` (+10, exercising every NINO prefix rule directly), `test-no-false-positives.sh` (+4 precision guards for excluded prefixes and unspaced-postcode collisions), and three new `tests/corpus.json` cases (a NINO gold, a postcode gold, and a preserve case locking in the unspaced-postcode suppression). The corpus ratchet stays at recall 1.0 / 0 preserve violations.
